...
(async () => {
    ...
    page.on('response', async (response) => {
        ...
        if (contentType.includes('application/json') && (url.includes('api') || url.includes('products') || url.includes('search') || url.includes('catalog'))) {
            try {
                const text = await response.text();
                if (text.length > 500) { // Likely contains interesting data if it's large
                    console.log(`\n\n=== FOUND JSON API: ${url} ===`);
                    fs.appendFileSync('automercado_api_log.txt', `URL: ${url}\n\nRESPONSE:\n${text.substring(0, 1000)}\n\n`);
                }
            } catch (e) {
                // ignore
            }
